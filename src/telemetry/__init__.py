...
def setup_telemetry(service_name: str = "cognivault-engine") -> None:
...
def _build_otlp_exporter() -> Optional[OTLPSpanExporter]:
    ...
    if not endpoint:
        return None
    # gRPC exporter uses endpoint directly, headers are passed via metadata
    headers = os.getenv("OTEL_EXPORTER_OTLP_HEADERS")
    metadata = None
    if headers:
        # Parse headers into gRPC metadata format (list of tuples)
        metadata = [
            tuple(item.split("=", 1)) for item in headers.split(",") if "=" in item
        ]
    return OTLPSpanExporter(endpoint=endpoint, metadata=metadata)
